...
//set vars as empty
$colourID = $sizeID = $email = $artUpload = $productID = $orderQty = $orderPrice = '';
//error vars
$colourErr = $sizeErr = $orderQtyErr = $emailErr = $uploadOk = $imgErr = $imgUploadErr ='';
...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
 ...
 if(!is_numeric($_POST['orderQty'])){ //check Qty input is numeric only
 $orderQtyErr = "<p>This field accepts numeric values only.</p>";
 } else {
 $orderQty = test_input($_POST['orderQty']);
 }

 $email = test_input($_POST['email']);//validated front end
 $productID = test_input($_POST['productID']);
 $price = test_input($_POST['pricePerItem']);//used to calculate orderPrice only

 $orderPrice = $price * $orderQty ;// prderPrice = pricePerItem * orderQty from user inputs
 ...
 if(empty($_FILES["artUpload"]['tmp_name'])){ // bespoke warning if file upload is empty
 ...
 if ($uploadOk == 0 or $colourErr or $sizeErr) { //if errors exist do not upload
...
if($productID != '' && $orderQty != '' && $artUpload != '' && $email != ''){ //conditions to process

//test outputs before processing
echo "<p>Quote requested</p>";

echo '<p>prodID | ' . $productID . ' </p> ';
echo '<p>qty | ' . $orderQty . ' </p> ';
echo '<p>total price @ RRP | $' . $orderPrice . ' </p> ';
echo '<p>artwork provided \'uplads/' . $artUpload . '\'</p> ';

echo '<p>requested by |' . $email . ' </p> ';
